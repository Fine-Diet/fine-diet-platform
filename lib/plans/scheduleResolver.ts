/**
 * Plans — Schedule Resolver (Phase 3 → Meal Rhythm v2)
 *
 ...
 *
 * Contract (locked in Packet 3, evolved in Meal Rhythm v2A):
 *   - Profile owns the baseline meal schedule template (v2 occasions).
 ...
 *   - Enabled occasions display chronologically by target_time.
 *
...
interface MergedOverrides {
  require_slots: Set<MealOccasionKey>;
  disallow_slots: Set<MealOccasionKey>;
  constraints: {
...
function mergeOverrides(overrides: ProgramScheduleOverride[]): MergedOverrides {
  const require_slots = new Set<MealOccasionKey>();
  const disallow_slots = new Set<MealOccasionKey>();
  ...
  for (const ov of overrides) {
    const normalized = normalizeProgramScheduleOverride(ov) ?? ov;
    for (const k of normalized.require_slots ?? []) require_slots.add(k);
    for (const k of normalized.disallow_slots ?? []) disallow_slots.add(k);
    const c = normalized.constraints ?? null;
    if (!c) continue;
...
export function resolveMealSchedule(
  ...
  const profile_schedule = normalizeMealSchedule(input.profile_schedule);
  ...
  for (const key of MEAL_OCCASION_KEYS) {
    const profileSlot = profile_schedule.slots[key];
    ...
    if (required && disallowed) {
      ...
      conflicts.push({
        ...
        slot_key: key,
        message: `One program requires ${labelForOccasion(key, profileSlot.label)} while another disallows it. The required slot wins.`,
      ...
      if (profileSlot.enabled) {
        conflicts.push({
          kind: 'required_vs_disabled',
          slot_key: key,
          message: `You have ${labelForOccasion(key, profileSlot.label)} enabled but a program disallows it. The program wins for this plan.`,
    ...
    entries.push({
      ...
      enabled,
      target_time,
      label: labelForOccasion(key, profileSlot.label),
...
export function buildPlanScheduleSnapshot(
  ...
  const normalized = normalizeMealSchedule(profile_schedule);
  ...
  return { profile_schedule: normalized, resolved_slots, conflicts };
}
