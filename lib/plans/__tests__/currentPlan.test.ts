...
describe('selectCurrentPlan', () => {
  ...
  it('uses id as stable final tie-breaker when timestamps match', () => {
    const a = plan({
      id: 'plan-a',
      status: 'active',
      created_at: '2026-07-31T12:00:00.000Z',
    ...
    expect(selectCurrentPlan([a, b])?.id).toBe('plan-b');
  });
...
describe('resolveCurrentPlan', () => {
  it('surfaces integrity conflict metadata for multiple actives', () => {
    const older = plan({
      id: 'older',
      status: 'active',
      created_at: '2026-07-20T09:00:00.000Z',
    ...
    const resolution = resolveCurrentPlan([older, newer]);
    expect(resolution.plan?.id).toBe('newer');
    expect(resolution.activeCount).toBe(2);
    expect(resolution.integrityConflict).toBe(true);
    expect(resolution.conflictPlanIds).toEqual(['newer', 'older']);
  });

  it('reports no conflict for a single active plan', () => {
    const resolution = resolveCurrentPlan([
      plan({ id: 'only', status: 'active' }),
    ]);
    expect(resolution.integrityConflict).toBe(false);
    expect(resolution.conflictPlanIds).toEqual([]);
  });
});
