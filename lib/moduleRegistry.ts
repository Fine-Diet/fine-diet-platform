...
export const MODULE_STYLE_CATALOG: ModuleDefinition[] = [
  ...
  /* ── Slot Card ─────────────────────────────────────────────────── */
  {
    ...
    notes:
      'Cataloged in Packet 2C-B; live-previewable with PlanSlot + PlannedMeal[] fixtures. All callbacks are no-ops in preview; internal links use the real route builders but are not navigated. Manages its own inline time-edit state. Truth logic (NDS, readiness, execution) lives in the parent page and is untouched. Style reference only (Packet 2F): an internal renderer on the /journal/plans/[date] detail surface, not a canonical main-page module. Use for spacing/taste guidance; not approved as a new-page foundation until the Plans detail surface is designed and accepted.',
    status: 'stable',
    lifecycle: 'reference_only',
    surface: 'signed_in_app',
    reusability: 'needs_data',
  ...
  /* ── Day View ──────────────────────────────────────────────────── */
  {
    ...
    notes:
      'Cataloged in Packet 2C-B; live-previewable with PlanDay + PlanSlot[] + PlannedMeal[] fixtures. Orchestrates SlotCard; all callbacks are no-ops in preview. Slot ordering + meal grouping happen client-side from props (no fetch). Style reference only (Packet 2F): lives on the /journal/plans/[date] detail route, not the canonical main Plans page. Use for spacing/taste guidance; not approved as a new-page foundation until the Plans-day detail surface is designed and accepted. (Product review: candidate to re-approve as the canonical Plans-day pattern.)',
    status: 'stable',
    lifecycle: 'reference_only',
    surface: 'signed_in_app',
    reusability: 'needs_data',
  ...
  /* ── Program Delivery Modules ──────────────────────────────────── */
  {
    ...
    variants: ['default'],
    notes:
      'Cataloged in Packet 2C-B; live-previewable with a ProgramDeliveryModuleDefinition[] fixture + runtimeSummary=null (status "not_started"). Fixture modules omit day bounds and include "not_started" visibility so they render without enrollment. CTA hrefs are inert. Visibility/copy helpers are pure functions (no API). Style reference only (Packet 2F): a program-detail renderer for /journal/programs/[slug], not a canonical main-page module. Use for spacing/taste guidance; not approved as a new-page foundation until the program-detail surface is designed and accepted.',
    status: 'stable',
    lifecycle: 'reference_only',
    surface: 'signed_in_app',
    reusability: 'needs_data',
  ...
  /* ── Baseline Prep Modules ─────────────────────────────────────── */
  {
    ...
    notes:
      'Cataloged in Packet 2C-B; live-previewable with access="primary"/"reference" and null runtime/progress summaries (detail pills show "Not enrolled"/"Not set"). access="hidden" renders nothing, so it is not a variant. Import link uses the real route but is not navigated. No program truth is altered. Style reference only (Packet 2F): a baseline-detail renderer for /journal/programs/baseline, not a canonical main-page module. Use for spacing/taste guidance; not approved as a new-page foundation until the baseline-detail surface is designed and accepted.',
    status: 'stable',
    lifecycle: 'reference_only',
    surface: 'signed_in_app',
    reusability: 'needs_data',
