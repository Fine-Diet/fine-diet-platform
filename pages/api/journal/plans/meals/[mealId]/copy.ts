/**
 * POST /api/journal/plans/meals/:mealId/copy
 *
 * Packet 41 — duplicate one planned meal into another day/slot. This creates
 * a fresh pending planned_meal row; source execution history stays untouched.
 */
...
import { copyPlannedMeal } from '@/lib/plans/planServerService';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await copyPlannedMeal(
      personId,
      mealId,
      targetPlanDayId,
      targetPlanSlotId,
    ...
    console.error('[API /journal/plans/meals/:mealId/copy] error:', err);
