...
const PILL_BASE =
  'rounded-full bg-gradient-to-bl from-denim-500 to-denim-900 px-8 py-4 text-center text-base font-semibold text-neutral-900 antialiased transition-opacity duration-200 hover:opacity-90 sm:py-5';
...
export function PrimaryPillCta({
  ...
  if (cta.href && !cta.disabled) {
    return (
      <Link href={cta.href} className={`${PILL_BASE} ${width} ${className}`}>
        {cta.label}
      </Link>
    );
  }
  ...
  return (
    <span
      className={`${PILL_BASE} ${width} cursor-not-allowed opacity-60 ${className}`}
      aria-disabled="true"
    >
      {cta.label}
    </span>
