...
export function PlanningRouteRail({
  ...
  return (
    <nav
      ...
      className="relative z-[1] border-y border-white/20 bg-[#1c1712]"
    >
      <div className="flex overflow-x-auto snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {items.map((item, index) => (
          <Link
            ...
            href={item.href}
            className={cn(
              'relative flex min-w-[11rem] flex-1 snap-start items-center justify-center px-4 py-4 text-center text-sm font-semibold antialiased transition-colors sm:min-w-0',
              item.active
