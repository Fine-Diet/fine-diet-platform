...
BEGIN
  -- Get person_id and date from the entry
  IF TG_OP = 'DELETE' THEN
    person_uuid := OLD.person_id;
    entry_date := (OLD.occurred_at AT TIME ZONE 'UTC')::DATE;
  ELSE
    person_uuid := NEW.person_id;
    entry_date := (NEW.occurred_at AT TIME ZONE 'UTC')::DATE;
  END IF;
  
  -- Schedule for 5 seconds from now (debounce window)
  scheduled_time := NOW() + INTERVAL '5 seconds';
  
  -- Upsert into queue (update scheduled_for if pending exists, insert otherwise)
  INSERT INTO public.nds_recompute_queue (person_id, date_local, scheduled_for)
  VALUES (person_uuid, entry_date, scheduled_time)
