...
--   Application fallback (if RPC absent): activate the new plan first, then
--   best-effort archive prior actives. Never archive/retire a valid current
--   plan before the generated plan is confirmed active. On child-write failure
--   discard the incomplete draft only — never mutate prior actives.
-- ============================================================================
...
BEGIN
  ...
  -- Promote the generated plan first so a prior current plan is never
  -- retired into a zero-active window inside this transaction.
  UPDATE public.plans
    SET status = 'active',
        updated_at = now()
    WHERE id = p_plan_id
